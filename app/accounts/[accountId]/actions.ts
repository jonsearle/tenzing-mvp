...
export async function saveDecisionAction(
  ...
  await requireUser(`/accounts/${context.accountId}`);
  ...
  if (!isActionLabel(selectedAction)) {
    redirect(
      `/accounts/${context.accountId}?decisionStatus=missing-action`,
    );
  }
  ...
  if (result.status === "saved") {
    redirect(`/accounts/${context.accountId}?decisionStatus=saved`);
  }

  if (result.status === "unavailable") {
    redirect(`/accounts/${context.accountId}?decisionStatus=unavailable`);
  }

  redirect(`/accounts/${context.accountId}?decisionStatus=error`);
}
